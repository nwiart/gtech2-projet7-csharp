...
namespace ConsoleGame.Beast
{
	public class Beast
	{
		...
		static Beast()
		{
			...
			registerBeast("leggedthing", new Beast("Truc à Pats", 1, 5, 5, 10, 10, leggedCapacity));
			registerBeast("ambush", new Beast("Embuisscade", 1, 10, 10, 10, 10, ambushCapacity));
			registerBeast("papiermachette", new Beast("Origamonstre", 1, 15, 15, 10, 10, papiermachetteCapacity));
		...
		Beast(string name, int level, int attack, int defense, int maxHealth, int maxMana, Capacity[] capacities)
		{
			RegistryID = null;
			Name = name;
            Level = level;
